...
function CameraCapture({ onCapture, onCancel }) {
  ...
  const processImageWithOpenCV = useCallback((imageCanvas) => {
    ...
    try {
      ...
      const corners = [];
      for(let i=0; i<docContour.data32S.length; i+=2) corners.push({x: docContour.data32S[i], y: docContour.data32S[i+1]});
      ...
      const dstTri = cv.matFromArray(4, 1, cv.CV_32FC2, [0, 0, maxWidth - 1, 0, 0, maxHeight - 1, maxWidth - 1, maxHeight - 1]);
      ...
      src.delete(); gray.delete(); blurred.delete(); edged.delete(); contours.delete(); hierarchy.delete(); docContour.delete(); M.delete(); warpedMat.delete(); srcTri.delete(); dstTri.delete();
  ...
  const detectLoop = useCallback(async () => {
    if (!modelRef.current || !videoRef.current || !videoRef.current.srcObject || isCapturing) return;
  ...
  return (
    <div className="relative w-full h-[80vh] bg-black rounded-lg">
      ...
      <video ref={videoRef} autoPlay playsInline muted className={`w-full h-full object-cover transition-opacity duration-300 ${isReady ? 'opacity-100' : 'opacity-0'}`} />
      <button onClick={onCancel} className="absolute top-4 right-4 bg-white bg-opacity-80 text-black rounded-full w-10 h-10 flex items-center justify-center font-bold text-xl shadow-lg z-20">×</button>
    </div>
  );
}
