...
const initialTickets = [
  ...
  },
  {
    ...
    description: 'New mockups are available.',
    status: 'In Progress',
    priority: 'Medium',
  ...
  },
  {
    ...
    description: 'The "anual" plan should be "annual".',
    status: 'Closed',
    priority: 'Low',
...
const initialMaintenanceTasks = [
  ...
  },
  {
    ...
    interval: 'weekly',
    nextRun: new Date(new Date().getTime() + 7 * 24 * 60 * 60 * 1000),
  },
];
...
const theme = createTheme({
  ...
  components: {
    ...
    MuiButton: {
      styleOverrides: {
        root: { textTransform: 'none', borderRadius: '8px', fontWeight: 600, padding: '8px 16px' },
      },
    },
    MuiTooltip: {
      styleOverrides: { tooltip: { backgroundColor: '#1e293b', borderRadius: '6px' } },
    },
  },
});
...
const getPriorityStyle = (priority) => {
  switch (priority) {
    case 'High':
      return { borderLeft: `4px solid ${theme.palette.error.main}` };
    case 'Medium':
      return { borderLeft: `4px solid ${theme.palette.warning.main}` };
    case 'Low':
      return { borderLeft: `4px solid ${theme.palette.primary.light}` };
    default:
      return {};
  }
};
              ...
              <Grid container spacing={2}>
                <Grid xs={12}>
                  <TextField
                    ...
                    onChange={handleFormChange(setNewTicket)}
                    required
                  />
                </Grid>
                <Grid xs={12}>
                ...
                <Grid xs={12} md={4} container justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={2}>
                    <Button variant="text" onClick={() => setIsFormVisible(false)}>
                      Cancel
                    </Button>
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid xs={12} sm={4} md={3}>
                  <DatePicker
                    ...
                    onChange={(newValue) => setDateFilter((p) => ({ ...p, start: newValue }))}
                ...
                <Grid xs={12} sm={4} md={3}>
                  <DatePicker
                    ...
                    onChange={(newValue) => setDateFilter((p) => ({ ...p, end: newValue }))}
                    slotProps={{ textField: { size: 'small', fullWidth: true } }}
                  />
                </Grid>
                <Grid
            ...
            <Stack spacing={2.5}>
              {filteredTickets.length > 0 ? (
                filteredTickets.map((ticket) => (
                  <Paper
                    key={ticket.id}
                    sx={{ ...getPriorityStyle(ticket.priority), overflow: 'hidden' }}
                  >
                    <Box sx={{ p: 2.5 }}>
                      ...
                      <Grid container spacing={2} alignItems="center">
                        <Grid xs={12} md={6}>
                          ...
                          >
                            <Chip
                              icon={<User size={16} />}
                              label={`By: ${ticket.createdBy}`}
                              size="small"
                              variant="outlined"
                            />
                            <Chip
                              icon={<CalendarClock size={16} />}
                              label={ticket.createdAt.toLocaleDateString()}
                              size="small"
                              variant="outlined"
                            />
                          </Stack>
              ...
              <form onSubmit={handleCreateMaintenanceTask}>
                <Stack spacing={2}>
                  <TextField
                    ...
                    label="New Task Name"
                    name="name"
                    value={newMaintenanceTask.name}
                    onChange={handleFormChange(setNewMaintenanceTask)}
                    required
                    size="small"
                  />
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      label="Interval"
                      name="interval"
                      value={newMaintenanceTask.interval}
                      onChange={handleFormChange(setNewMaintenanceTask)}
                    >
                  ...
                  <Button type="submit" variant="outlined" startIcon={<ChevronsRight size={16} />}>
                    Schedule Task
                  </Button>
                </Stack>
              </form>
